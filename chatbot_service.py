...
from database import get_db_session, User, Conversation, ChatMessage, DiseaseDetection
from main.graph import app as langgraph_app  # Đảm bảo đường dẫn này đúng


class AgricultureChatbot:
    """Async Service Layer for chatbot logic."""

    def __init__(self, db: AsyncSession = Depends(get_db_session)):
        self.db = db
        self.graph = langgraph_app

    # --- HÀM get_or_create_conversation GIỮ NGUYÊN ---
    ...
    # --- HÀM save_user_message GIỮ NGUYÊN (NHƯNG BỊ LỖI THIẾU user_id) ---
    # LƯU Ý: Hàm này của bạn đang thiếu user_id, tôi sẽ sửa nó luôn
    async def save_user_message(self, user_id: int, conversation_id: str, content: str):
        """Lưu tin nhắn của người dùng vào DB."""
        user_msg = ChatMessage(
            user_id=user_id,  # <-- THÊM DÒNG NÀY (Sửa lỗi từ trước)
    ...
    # --- HÀM _parse_confidence GIỮ NGUYÊN ---
    ...
    # --- HÀM process_query ĐÃ ĐƯỢC VIẾT LẠI HOÀN TOÀN ---
    async def process_query(self, user_id: int, user_query: str, conversation_id: str,
        ...
        try:
            # Sửa lỗi từ trước: truyền user_id vào
        ...
        inputs = {
            # "user_query" không còn cần thiết ở đây vì nó đã có trong 'messages'
        ...
        # 2. KHỐI LỆNH TRY...EXCEPT CHÍNH
        try:
            # --- THAY ĐỔI LỚN: DÙNG ainvoke THAY VÌ astream_events ---
            # Chạy toàn bộ graph và chờ kết quả cuối cùng
            print("Đang gọi graph.ainvoke...")
            final_state = await self.graph.ainvoke(inputs, config)
            print("Graph đã chạy xong.")

            # Lấy nội dung phản hồi cuối cùng từ state
            if final_state:
            ...
            # --- TOÀN BỘ LOGIC LƯU DB TỪ on_graph_end ĐƯỢC CHUYỂN VỀ ĐÂY ---
            bot_msg_id = None
                    ...
                    if query_type == 'image_disease' and info:
                        ...
                        if disease_name and disease_name not in ["Analysis inconclusive", "Error processing image"]:
                            ...
                            detection = DiseaseDetection(
                                ...
                                plant_type=info.get('plant_type'),
            ...
            # 4. GỬI SỰ KIỆN KẾT THÚC (CHỈ 1 LẦN)
            # Gửi toàn bộ phản hồi trong sự kiện 'end'
            yield f"data: {json.dumps({'event': 'end', 'final_message': final_bot_response, 'conversation_id': conversation_id})}\n\n"

        except Exception as e:
            # Bắt lỗi từ graph.ainvoke hoặc lỗi DB
