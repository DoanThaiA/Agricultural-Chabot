...
def handle_login(username: str, password: str) -> bool:
    ...
    if response and response.status_code == 200:
        ...
        error_msg = response.json().get('detail', 'Sai tên đăng nhập hoặc mật khẩu') if response else 'Lỗi kết nối'
...
def handle_register(username: str, email: str, password: str) -> bool:
    ...
    if response and response.status_code == 201:
        ...
        error_msg = response.json().get('detail', 'Lỗi đăng ký') if response else 'Lỗi kết nối'
        st.sidebar.error(f"❌ {error_msg}")
        return False
...
def handle_logout():
    ...
    for key in ["user_id", "username", "messages", "conversation_id", "conversation_list", "disease_history"]:
        st.session_state[key] = [] if key in ["messages", "conversation_list", "disease_history"] else None
    st.session_state.view_mode = "chat"
...
def render_auth_sidebar():
    ...
    with tab1:
        ...
        with st.form("login_form", clear_on_submit=False):
            ...
            with col2:
                clear_btn = st.form_submit_button("Xóa", use_container_width=True)
    ...
    if not st.session_state.disease_history:
        ...
        for item in filtered_data:
            data_to_display.append({
                ...
                "Loại cây": item.get("plant_type", "N/A"),
        ...
        if not data_to_display:
            ...
            column_config = {
                ...
                "Loại cây": st.column_config.TextColumn(
                    "🌱 Loại cây",
                    width="medium"
                )
            }
